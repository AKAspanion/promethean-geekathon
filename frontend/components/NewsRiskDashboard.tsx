    ...
    <div className="rounded-xl border border-light-gray dark:border-gray-600 bg-white dark:bg-gray-800 p-4 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="flex items-start justify-between gap-3 mb-2">
        ...
        <div className="flex shrink-0 flex-wrap gap-1.5">
          <span
            className={`rounded-md px-2 py-0.5 text-[11px] font-medium ${severityColors[risk.severity] ?? defaultBadge}`}
          >
            {risk.severity}
          </span>
          <span
            className={`rounded-md px-2 py-0.5 text-[11px] font-medium ${statusColors[risk.status] ?? defaultBadge}`}
          >
      ...
      <div className="flex flex-wrap items-center gap-2 text-[11px]">
        {riskType && (
          <span
            className={`rounded-md px-2 py-0.5 font-medium ${riskTypeBadge[riskType] ?? defaultBadge}`}
          >
        ...
        {source && (
          <span
            className="text-medium-gray dark:text-gray-400 truncate max-w-[140px]"
            title={source}
          >
            📰 {source}
          </span>
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-light-gray dark:border-gray-600 py-16 text-center">
      <div className="rounded-full bg-sky-blue/30 dark:bg-gray-700/50 p-3">
        <svg
          className="h-8 w-8 text-primary-dark dark:text-primary-light"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
      ...
      <div className="flex items-center justify-between gap-2 mb-3">
        <div>
          ...
          <div className="text-[11px] text-medium-gray dark:text-gray-400">
            {risks.length} risk record{risks.length !== 1 ? "s" : ""} ·
            sourceType: news
          </div>
        </div>
        <button
...
function horizonKey(horizon: string | null | undefined) {
  ...
  if (h.includes("short") || h.includes("immediate") || h.includes("week"))
    return "short";
  ...
  return (
    <div className="rounded-xl border border-light-gray dark:border-gray-600 bg-white dark:bg-gray-800 p-4 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="flex flex-wrap items-center gap-2 text-[11px] mb-2">
        {insight.scope && (
          <span
            className={`rounded-md px-2 py-0.5 font-medium ${scopeColors[insight.scope] ?? defaultBadge}`}
          >
            {insight.scope}
          </span>
        )}
        ...
        {insight.time_horizon && (
          <span
            className={`rounded-md px-2 py-0.5 font-medium ${horizonColors[hKey] ?? defaultBadge}`}
          >
            ⏱ {insight.time_horizon}
          </span>
        )}
      ...
      {insight.predicted_impact && (
        <div className="mb-2 rounded-lg bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/30 px-3 py-2 text-[12px] text-amber-800 dark:text-amber-300">
          <span className="font-semibold">Predicted impact: </span>
          {insight.predicted_impact}
        </div>
      )}
        ...
        insight.recommended_actions.length > 0 && (
          <div>
            ...
            >
              ...
              {expanded ? "Hide" : "Show"} {insight.recommended_actions.length}{" "}
              recommended action
              {insight.recommended_actions.length !== 1 ? "s" : ""}
            </button>
            {expanded && (
              <ul className="mt-2 space-y-1.5 pl-2">
                {insight.recommended_actions.map((action, i) => (
                  <li
                    key={i}
                    className="flex gap-2 text-[12px] text-dark-gray dark:text-gray-300"
                  >
...
function TrendSummary({ insights }: { insights: TrendInsightItem[] }) {
  ...
  const opps = insights.filter(
    (i) => i.risk_opportunity === "opportunity",
  ).length;
  ...
  return (
    <div className="flex flex-wrap gap-4">
      ...
      {Object.entries(byScope).map(([scope, n]) => (
        <span
          key={scope}
          className={`rounded-md px-2 py-0.5 text-[11px] font-medium ${scopeColors[scope] ?? defaultBadge}`}
        >
          {n} {scope}
        </span>
      ))}
...
function TrendDevView({
  result,
  copied,
  onCopy,
}: {
  result: TrendInsightRunResult;
  copied: boolean;
  onCopy: () => void;
}) {
  return (
    <div className="rounded-xl border border-light-gray dark:border-gray-600 bg-off-white dark:bg-gray-700/50 p-4">
      <div className="flex items-center justify-between gap-2 mb-3">
        <div>
          ...
          <div className="text-[11px] text-medium-gray dark:text-gray-400">
            {result.insights_generated} insight
            {result.insights_generated !== 1 ? "s" : ""} · provider:{" "}
            {result.llm_provider}
          </div>
        </div>
        <button
...
export function NewsRiskDashboard() {
  ...
  const [lastNewsResult, setLastNewsResult] = useState<{
    risksCreated: number;
    opportunitiesCreated: number;
  } | null>(null);

  // ── Trend insights ─────────────────────────────────────────────────────────
  const [trendResult, setTrendResult] = useState<TrendInsightRunResult | null>(
    null,
  );
  ...
  async function handleRunTrendAnalysis() {
    ...
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : "Trend analysis failed. Check server logs.";
  ...
  function handleTrendsCopy() {
    ...
    navigator.clipboard
      .writeText(JSON.stringify(trendResult, null, 2))
      .then(() => {
  ...
  const risks = selectedSupplier
    ? allRisks.filter(
        (r) =>
          (r.affectedSupplier &&
            r.affectedSupplier.toLowerCase() ===
