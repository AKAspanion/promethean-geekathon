...
function SwarmSummary({ swarm }: { swarm: SupplierSwarmSummary | null }) {
  ...
  const primaryAgent =
    swarm.agents.find((a) => a.riskLevel === swarm.riskLevel) ??
    swarm.agents[0];
    ...
    <div className="mt-2 space-y-1">
      ...
      {primaryAgent && (
        <p className="text-xs text-medium-gray dark:text-gray-400 truncate max-w-[260px]">
          ...
          {swarm.topDrivers && swarm.topDrivers.length > 0
            ? ` - ${swarm.topDrivers[0]}`
            : ""}
        </p>
      )}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                  <div className="flex-1">
                    ...
                    <p className="mt-1 text-xs text-medium-gray dark:text-gray-400">
                      Upload a CSV file with supplier details to quickly
                      populate your watchlist. Required columns should match the
                      backend schema.
                    </p>
                    {selectedFile && (
            ...
            <div className="p-12">
              <div className="animate-pulse space-y-4">
                {[1, 2, 3, 4, 5].map((i) => (
                  <div
                    key={i}
                    className="h-14 bg-light-gray dark:bg-gray-700 rounded"
                  />
                ))}
              </div>
            </div>
